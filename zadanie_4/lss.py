...
def get_polynomial_form(polynomial_degree: int) -> np.ndarray:
    ...
    polynomial_form = [[i] for i in range(polynomial_degree + 1)]
    return np.array(polynomial_form)
...
def print_polynomial(theta: np.ndarray, precission: int = 3) -> str:
    ...
    for i, coeff in enumerate(theta):
        if i == 0:
            polynomial_str += f"{coeff:.{precission}f}"
        else:
            if coeff >= 0:
                polynomial_str += f" + {coeff:.{precission}f}x^{degree - i}"
            else:
                polynomial_str += f" - {abs(coeff):.{precission}f}x^{degree - i}"
...
def least_squares_solution(
    ...
    """
    polynomial_form = get_polynomial_form(polynomial_degree)


    X_poly = np.column_stack([X ** deg for deg in polynomial_form])
